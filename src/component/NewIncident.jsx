...
function NewIncident() {
  ...
  useEffect(() => {
    ...
    setItems([...new Set(NewIncidentItem.map((item) => item.DataTransferItemList))]);
  }, []);
      ...
      <div className="grid grid-cols-3 sm:grid-cols-1 lg:grid-cols-3 gap-4">
        {data.map((item) => (
          <div key={item.id} className="galleryItem flex flex-col items-center border p-2 rounded-lg">
            <img src={item.img} alt="" className="mb-2 w-full h-auto rounded" />
      ...
      <div className="mt-6">
        <ul className="flex flex-wrap gap-2 justify-center">
          ...
          {location.pathname === "/newincident/new" ? (
            <button 
              onClick={() => navigate("/newincident/new/dataitem")} 
